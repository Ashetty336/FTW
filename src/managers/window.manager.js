...
class WindowManager {
  ...
  toggleLLMResponseVisibility() {
    ...
    if (!this._llmHasContent) {
      logger.info('LLM response toggle ignored - no content yet (use Shift+B first)');
      return;
    }

    if (this.isScreenBeingShared) {
      logger.warn('LLM response toggle blocked due to screen sharing mode');
      return;
    }
  ...
  calculateOptimalWindowSize(contentMetrics) {
    // Fixed window size. Content scrolls inside the window instead of the
    // window growing/shrinking to fit the response.
    return { width: 840, height: 480 };
  }
