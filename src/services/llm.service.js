...
class LLMService {
  ...
  formatImageInstruction(activeSkill, programmingLanguage) {
    const isDsa = String(activeSkill).toLowerCase() === 'dsa';
    // For DSA the on-screen template decides the language, so only state a
    // default here rather than forcing one.
    const langNote = programmingLanguage
      ? (isDsa
          ? ` If no template is visible, default to ${programmingLanguage.toUpperCase()}.`
          : ` Use only ${programmingLanguage.toUpperCase()} for any code.`)
      : '';
    if (isDsa) {
      return `Read the question in this image and answer it following your system instructions exactly.

If it is a programming problem (input/output examples, constraints, or a visible code editor / function template): output EXACTLY ONE fenced code block and NOTHING else - no explanation, no approach, no headings. Match the language of any template visible on screen, otherwise use C++. The code MUST be properly formatted with real line breaks: one statement per line, newline after every "{", before every "}", and after every ";", nested blocks indented 4 spaces. NEVER put the whole solution on one line. No comments except the final two lines giving time and space complexity.

Otherwise (aptitude, MCQ, fill in the blank, true/false, theory, system design, output prediction): output ONLY the bare answer - the option letter and text for MCQs, just the word for fill-in-the-blanks, just the number for numeric answers. No explanation, no reasoning, no working, no preamble.${langNote}`;
    }
  ...
  enforceProgrammingLanguage(text, programmingLanguage) {
    try {
      ...
      // Only fill in a language tag when the model left the fence bare. A tag the
      // model chose deliberately (e.g. it matched an on-screen Python template)
      // must be preserved, otherwise highlighting and the language are wrong.
      const knownTags = ['cpp', 'c', 'python', 'py', 'java', 'javascript', 'js', 'typescript', 'ts',
        'csharp', 'cs', 'go', 'rust', 'rs', 'php', 'kotlin', 'kt', 'swift', 'scala', 'ruby', 'rb', 'sql'];

      const replacedBackticks = text.replace(/```([^\n]*)\n/g, (match, info) => {
        const current = (info || '').trim().split(/\s+/)[0].toLowerCase();
        if (current && knownTags.indexOf(current) !== -1) return match;
        return '```' + fenceTag + '\n';
      });

      const normalizedTildes = replacedBackticks.replace(/~~~([^\n]*)\n/g, (match, info) => {
        const current = (info || '').trim().split(/\s+/)[0].toLowerCase();
        if (current && knownTags.indexOf(current) !== -1) return '```' + current + '\n';
        return '```' + fenceTag + '\n';
      });
