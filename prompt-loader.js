...
class PromptLoader {
  ...
  injectProgrammingLanguage(promptContent, programmingLanguage, skillName) {
    ...
    switch (skillName) {
      case 'dsa':
        // Applies to CODE MODE only. ANSWER MODE (aptitude, MCQ, theory,
        // system design) is unaffected and must still return a bare answer.
        languageInjection = `\n\n## DEFAULT IMPLEMENTATION LANGUAGE: ${languageUpper}
These rules apply ONLY when you are in CODE MODE. They do not apply in ANSWER MODE.
- If a function template, class stub, or code editor is visible in the question, write in THAT language and match its exact signature.
- If no template is visible, write in ${languageTitle} using the fence tag: \`\`\`${fenceTag}\`\`\`.
- Do not include snippets or alternatives in more than one language.
- Output EXACTLY ONE fenced code block. NO prose, NO approach text, NO headings, NO explanation before or after it.
- The code MUST be properly formatted with real line breaks: exactly ONE statement per line, a newline after every "{", before every "}", and after every ";", nested blocks indented 4 spaces per level. NEVER put the whole solution on one line or a few long lines. It must read like a normal source file.
- Aim for the best possible time and space complexity; prefer optimal algorithms and data structures.
- No comments except the final two lines, which state time complexity and space complexity.
- If the input is a problem statement without code, produce a complete, runnable solution without asking for clarification.`;
        break;
      default:
